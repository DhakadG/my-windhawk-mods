// ==WindhawkMod==
// @id              taskbar-clock-customization-v2
// @name            Taskbar Clock Customization v2
...
| `%media_info%` | Combined `Artist – Title` (auto‑truncated with ellipsis) |
...
std::wstring PadNumberWithFigureSpace(double val, unsigned int fixedDecimals, int minIntegerDigits)
{
...
void ComputeAutoAlignPadding()
{
    ...
    if (GlobalMemoryStatusEx(&memStatus))
    {
        ...
        g_autoRamIntDigits = (maxInt >= 100) ? 3 : (maxInt >= 10) ? 2
                                                                  : 1;
    }
    ...
    winrt::com_ptr<IDXGIFactory> factory;
    if (SUCCEEDED(CreateDXGIFactory(__uuidof(IDXGIFactory), factory.put_void())))
    {
        winrt::com_ptr<IDXGIAdapter> adapter;
        for (UINT i = 0;
             factory->EnumAdapters(i, adapter.put()) != DXGI_ERROR_NOT_FOUND; i++)
        {
            DXGI_ADAPTER_DESC desc{};
    ...
    if (maxVramBytes > 0)
    {
        ...
        // If total VRAM > 9999 MB (~10 GB), display in GB to avoid 5-digit numbers.
        ...
        if (g_autoVramInGb)
        {
            ...
            g_autoVramIntDigits = (maxGbInt >= 100) ? 3 : (maxGbInt >= 10) ? 2
                                                                           : 1;
        }
        else
        {
...
void FormatTransferSpeed(double val, PWSTR buffer, size_t bufferSize)
{
    ...
    {
        digitsAfterDecimal = (g_settings.dataCollection.networkMetricsFixedDecimals != -1)
                                 ? g_settings.dataCollection.networkMetricsFixedDecimals
                                 : 2;
    }
    else if (g_settings.dataCollection.networkMetricsFixedDecimals == -1)
    {
    ...
    std::wstring valUnitFormatted = g_settings.dataCollection.alignMetrics
...
PCWSTR GetRamGbFormatted()
{
    return GetMetricFormatted(
        g_ramGbFormatted, [](PWSTR buffer, size_t bufferSize)
        {
            ...
            DWORDLONG usedBytes = status.ullTotalPhys - status.ullAvailPhys;
