// ==WindhawkMod==
// @id              taskbar-fluent-media-player-fork
// @name            Taskbar Fluent Media Player - Fork
// @description     Fork of Salyts' Taskbar Fluent Media Player, fixed for Windows 11 26H2 (build 26300) where the system tray became a StackPanel. Integrates a modern Fluent Design media player directly into the Windows 11 taskbar.
// @description:ru-RU Форк мода Taskbar Fluent Media Player от Salyts, исправленный для Windows 11 26H2 (сборка 26300), где системный трей стал StackPanel. Встраивает современный медиаплеер в стиле Fluent Design прямо в панель задач Windows 11.
// @version         1.6.1
// @author          lost_husky
...
// ==WindhawkModReadme==
/*
# Taskbar Fluent Media Player - Fork 1.6.1

> **This is a fork.** The original
> [Taskbar Fluent Media Player](https://github.com/Salyts/Taskbar-Fluent-Media-Player)
> is by **[Salyts](https://github.com/Salyts)**, who wrote everything below and all of the
> code this is built on. This fork is maintained by
> [lost_husky](https://github.com/DhakadG) and exists for exactly one reason:
>
> **Windows 11 26H2 (build 26300) changed `SystemTrayFrameGrid` from a `Grid` to a
> `StackPanel`.** The upstream mod resolves its injection target with
> `try_as<Grid>()`, which now returns null, so every tray position fails and the player
> never appears — while still reporting a successful load. This fork adds a StackPanel
> path that inserts by child index instead of by reserved grid column, and leaves the
> pre-26300 Grid path untouched.
>
> Install the upstream mod instead once it carries the fix. Same root cause as
...
// FORK: `panel` replaces the old `Grid grid`. On Windows 11 26H2 (build 26300) the tray
// container SystemTrayFrameGrid kept its name but became a StackPanel, so the target is
// no longer guaranteed to be a Grid. Both are Panels.
//   insertCol >= 0 with a Grid panel  -> reserve a ColumnDefinition (every earlier build)
//   trayIndex >= 0                    -> insert at that index in Children() (26300+ tray)
struct InjectionTarget {
    Panel panel{nullptr};
    int   insertCol = 0;
    int   trayIndex = -1;
};
// FORK: takes a Panel. Grid.Column is only meaningful on a Grid parent; on a StackPanel
// the return value is -1 and callers must not touch column definitions with it.
static int RemovePlayerGridChildren(Panel const& targetGrid) {
    if (!targetGrid) return -1;
    bool isGrid = targetGrid.try_as<Grid>() != nullptr;
    int firstCol = -1;
...
static void RemoveAnchorDebugOverlays(Panel const& targetGrid) {  // FORK: Panel, see above
    ...
    std::wstring_view position)
{
    ...
    // FORK: Windows 11 26H2 (build 26300+). SystemTrayFrameGrid is a StackPanel here, so
    // the Grid block below is skipped entirely and every tray_* position used to fall
    // through to an empty target. Children are ordered by index; they do still carry
    // Grid.Column values, but those are leftovers and no longer drive layout, so the
    // index has to come from Children() order instead.
    if (auto trayPanel = trayFrame ? trayFrame.try_as<Panel>() : nullptr) {
        if (!trayPanel.try_as<Grid>()) {
            auto indexOf = [&](const wchar_t* name) -> int {
                auto children = trayPanel.Children();
            ...
            int count = (int)trayPanel.Children().Size();
            const wchar_t* anchor = nullptr;
            bool after = false;
            int  idx = -1;

            if      (position == L"tray_right")                    idx = count;
            else if (position == L"tray_left")                     idx = 0;
            ...
            if (anchor) {
                idx = indexOf(anchor);
                if (idx >= 0 && after) idx++;
                // tray_after_showdesktop_right kept its far-right fallback upstream.
                if (idx < 0 && position == L"tray_after_showdesktop_right") idx = count;
            }
            if (idx >= 0) {
                InjectionTarget t;
                t.panel = trayPanel;
                t.trayIndex = std::clamp(idx, 0, count);
                return t;
            }
            // Not a tray position - fall through to the taskbar handling below.
        }
    }
        ...
        position == L"taskbar_right_edge")
    {
        auto rootGrid = FindTaskbarRootGrid(root);
        if (!rootGrid) {
            // FORK: the tray fallback has to cope with either shape too.
            auto tf2 = FindChildByName(root, L"SystemTrayFrameGrid");
            if (auto tp2 = tf2 ? tf2.try_as<Panel>() : nullptr) {
                InjectionTarget t;
                t.panel = tp2;
                if (auto tg2 = tp2.try_as<Grid>()) {
                    t.insertCol = (int)tg2.ColumnDefinitions().Size();
                } else {
                    t.trayIndex = (int)tp2.Children().Size();
                }
                return t;
            }
            return {};
...
static bool InjectPlayerGrid() {
    ...
    try {
        ...
        auto [targetPanel, insertCol, trayIndex] = ResolveInjectionTarget(root, g_settings.position);
        if (!targetPanel) {
            if (g_settings.enableTreeDump) {
        ...
        // FORK: null when the target is the 26300+ tray StackPanel. Every use below that
        // needs real Grid semantics is now behind this.
        Grid targetGrid = targetPanel.try_as<Grid>();
        ...
        bool isTrayGrid = (targetPanel.Name() == L"SystemTrayFrameGrid");
        RemovePlayerGridChildren(targetPanel);
        RemoveAnchorDebugOverlays(targetPanel);

        // FORK: StackPanel tray. Order is the child index, so there is no column to
        // reserve and nothing to renumber. Re-resolve first - trayIndex was computed
        // before RemovePlayerGridChildren ran, and if our old element sat left of the
        // anchor everything after it has since shifted down by one.
        if (trayIndex >= 0) {
            auto fresh = ResolveInjectionTarget(root, g_settings.position);
            int at = (fresh.panel == targetPanel && fresh.trayIndex >= 0) ? fresh.trayIndex
                                                                         : trayIndex;
            at = std::clamp(at, 0, (int)targetPanel.Children().Size());
            ...
            targetPanel.Children().InsertAt((uint32_t)at, playerGrid);
            g_playerColumn = -1;
        }
        else if (isTrayGrid) {
        ...
        g_injectionParent = targetPanel;  // FORK: already a FrameworkElement, so Panel fits
        RefreshPlayerContents();
